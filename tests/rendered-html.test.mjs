...
test("renders the Chinese academic homepage with the requested publication labels", async () => {
  ...
  assert.equal((html.match(/class="publication-authors"/g) ?? []).length, 8);
  assert.equal((html.match(/<strong>(?:Xiuling Wang|Xiu-Ling Wang)<\/strong>/g) ?? []).length, 8);
  assert.match(html, /Intracellular and extracellular DNA archaeal communities.*?class="publication-authors"[^>]*>.*?<strong>Xiuling Wang<\/strong>.*?Thomas Friedl.*?Dirk Wagner.*?<\/p>/);
  assert.match(html, /href="https:\/\/link\.springer\.com\/journal\/248"/);
  assert.match(html, /<em>已投稿<\/em>/);
  assert.equal((html.match(/class="publication-manuscript"/g) ?? []).length, 1);
  ...
  const archaeaPosition = html.indexOf("Intracellular and extracellular DNA archaeal communities");
  ...
  assert.ok(archaeaPosition < bmcPosition && bmcPosition < rhizospherePosition && rhizospherePosition < microorganismsPosition);
...
test("renders a separate English route and local maintenance assets", async () => {
  ...
  assert.match(html, /<em>SUBMITTED<\/em>/);
  assert.match(html, /class="publication-authors"[^>]*>.*?<strong>Xiuling Wang<\/strong>.*?Gaodu Liang.*?Li Zhuang.*?<\/p>/);
  assert.equal((html.match(/class="publication-authors"/g) ?? []).length, 8);
  assert.equal((html.match(/<strong>(?:Xiuling Wang|Xiu-Ling Wang)<\/strong>/g) ?? []).length, 8);
