...
5. Once a DOI and bibliographic metadata are officially assigned, replace the journal-home link and add only confirmed details. Crossref/Elsevier now records the published Rhizosphere article as volume `39`, article `101421`, DOI `10.1016/j.rhisph.2026.101421`; no issue number is registered. The website card currently opens the official Elsevier author Share Link `https://authors.elsevier.com/a/1nS2y8jaVhezS3`, which provides free access through 4 September 2026. After that date, test the Share Link and replace it with the permanent DOI URL if it no longer resolves usefully.
6. Keep accepted work awaiting publication first, then sort published papers in reverse chronological order. The 2026 Rhizosphere paper therefore appears second, ahead of the earlier 2026 publication.
7. Every publication and accepted-manuscript card must include its complete verified author list. Use DOI/Crossref metadata for published articles, the journal PDF for the 2016 Pakistan Journal of Botany paper, and the current private manuscript package for accepted work without final bibliographic metadata. Highlight `Xiuling Wang`; preserve the published `Xiu-Ling Wang` spelling in the 2016 author list.
...
Current public status snapshot: BMC Microbiology accepted *Depth-dependent differences between direct total DNA and an intracellular-DNA-enriched fraction in bulk-soil bacterial and fungal communities of a Lonicera japonica field* on 17 July 2026. It is awaiting publication, has no DOI or final bibliographic details yet, and remains first on the website. Its author order is `Chenyi Mao, Xiuling Wang`, with Xiuling Wang identified as corresponding author. The Rhizosphere article is fully published in 2026 with Xiuling Wang listed as first and corresponding author, followed by Gaodu Liang and Li Zhuang. Its complete registered citation is *Rhizosphere* 39, 101421, DOI `10.1016/j.rhisph.2026.101421`; the card uses Elsevier's time-limited author Share Link and appears second. Affiliations and author email addresses are intentionally omitted from the compact public cards.
